...
class MultiPromptAttack(object):
    ...
    def run(self, 
        ...
        for i in range(n_steps):
            ...
            # Check if attack success needs to be tested immediately
            ...
            if stop_on_success:
                ...
                if all(all(tests for tests in model_test) for model_test in model_tests_jb):
                    ...
                    if verbose:
                        print(f"Attack succeeded! Jailbreak detected at step {i+1}")
            ...
            # Check if current step should be logged (original periodic logging logic OR jailbreak success)
            should_log = False
            if self.logfile is not None:
                # Original periodic logging logic
                if (i+1+anneal_from) % test_steps == 0:
                    should_log = True
                    if verbose:
                        print(f"Periodic log: Step {i+1}")
                
                # New: Log immediately when jailbreak succeeds
                elif jailbreak_success:
                    should_log = True
                    if verbose:
                        print(f"Success log: Jailbreak detected at step {i+1}, logging results immediately")
            ...
            if should_log:
                ...
                # If logged due to jailbreak success, terminate current query immediately
                if jailbreak_success:
                    if verbose:
                        print(f"Current query jailbreak succeeded, moving to next query...")
                    break
...
class ModelWorker(object):
    ...
    def run(model, tasks, results):
        while True:
            ...
            elif fn == "grad_with_pruning":
                with torch.enable_grad():
                    results.put(ob.grad_with_pruning(*args, **kwargs))
            else:
                with torch.no_grad():
                    if fn == "logits":
